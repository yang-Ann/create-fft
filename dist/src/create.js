...
export default class Create {
    ...
    initInstance() {
        ...
        this.templateFileMap = new TemplateFileMap(projectDir, language, templateParams);
...
- createDate: \`${new Date().toLocaleString().replace(/\//g, "-")}\``;
    ...
    cleanup() {
        ...
        if (!isCurrent)
            colorLog("blueBright", `\n cd ${lastDir}`);
        const { installCmd, runCmd } = this.getCmdTip();
        colorLog("blueBright", installCmd);
        colorLog("blueBright", runCmd);
    ...
    // 获取命令提示
    getCmdTip() {
        const pmn = whichPMRuns().name;
        let installCmd = "npm install";
        let runCmd = "npm run dev\n";
        if (pmn === "yarn") {
            installCmd = ` ${pmn} install`;
            runCmd = ` ${pmn} dev\n`;
        }
        else if (pmn === "pnpm") {
            installCmd = ` ${pmn} install`;
